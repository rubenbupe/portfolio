				...
				{active && typeof active === 'object' ? (
					<div className="fixed inset-0  grid place-items-center z-1">
						<motion.div
							...
							ref={ref}
							className="relative w-full max-w-[500px] flex flex-col  h-full md:h-fit md:max-h-[90%]  flex flex-col bg-card/90 sm:rounded-[48px] corner-squircle overflow-hidden sm:border sm:outline-solid outline-[1px] outline-background"
						>
							{animation}
							<motion.button
							...
							<div className="flex-1 flex flex-col overflow-auto [scrollbar-width:none] [-ms-overflow-style:none] [-webkit-overflow-scrolling:touch]">
								<div className="flex justify-between items-start p-4 px-6">
									<div className="">
										<motion.h3 layoutId={`title-${active.id}-${id}`} className="font-medium text-lg">
											{active.title}
										</motion.h3>
										<motion.p layoutId={`role-${active.id}-${id}`} className="text-primary/50 text-base">
											{active.role}
										</motion.p>
										<motion.p layoutId={`timeperiod-${active.id}-${id}`} className="text-primary/50 text-base text-xs">
											{active.timePeriod}
										</motion.p>
									</div>
								</div>
								<div className="p-4 px-6">
									<motion.div
										...
										animate={{ opacity: 1 }}
										exit={{ opacity: 0 }}
										className="flex gap-1 gap-y-2 flex-wrap"
									>
										{active.badges.map((badge, index) => (
											<span key={index} className="text-xs bg-foreground text-background px-2 py-1 rounded-full mr-2">
												{badge}
											</span>
										))}
									</motion.div>
					...
					>
						...
						>
							...
							>
								...
								<motion.p layoutId={`title-${card.id}-${id}`} className="text-white font-bold text-xl mt-4">
									{card.title}
								</motion.p>
								<motion.p className="absolute bottom-4 text-white/50 text-xs" layoutId={`timeperiod-${card.id}-${id}`}>
									{card.timePeriod}
								</motion.p>
								<p className="visually-hidden" aria-hidden="true">
									{card.paragraphs.reduce((acc, a) => `${acc} ${a}`, '')}
								</p>
							</HoloCard>
						</button>
					</motion.li>
